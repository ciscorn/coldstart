...
deploy: deploy-go deploy-bun deploy-rust deploy-gunicorn deploy-uvicorn deploy-node
...
deploy-node:
	docker buildx build ./node/ --platform linux/amd64 -t ${IMAGE_PREFIX}-node:latest
